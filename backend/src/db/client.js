...
export default class DB {
  ...
  async getRequests() {
    console.log('In constructor of getRequests()');
  ...
  async deleteWorker({ id = null }) {
    ...
    try {
      ...
      await this.#dbClient.query('DELETE FROM WORKERS WHERE ID = $1;', [id]);
  ...
  async updateWorker({ id = null, fio = '' }) {
    ...
    try {
      await this.#dbClient.query('UPDATE WORKERS SET FIO = $1 WHERE ID = $2;', [
        fio,
        id,
      ]);
    } catch (error) {
  ...
  async reassignTask({
    ...
    try {
      ...
      await this.assignTask({ workerId: dest_wordker_id, taskId });
    } catch (error) {
